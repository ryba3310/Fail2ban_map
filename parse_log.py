...
try:
    ...
    cur = conn.cursor()
    cur.execute('CREATE TABLE IF NOT EXISTS test4 (id SERIAL, ip VARCHAR(15) NOT NULL, banned BOOLEAN DEFAULT true NOT NULL, country VARCHAR(255) NOT NULL, region_name VARCHAR(255),\
...
def insert_host(host_metadata, attempt_data):
    ...
    print(host_metadata)
    try:
        cur.execute('INSERT INTO test4 (ip, banned, country, region_name, city, lat, lon, isp, org, asn, date, jail, attempts_num) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)\
...
def check_if_inserted(attempt_data):
    ...
    if record == None:
        return False
    cur.execute('UPDATE test4 SET attempts_num = attempts_num + 1 WHERE date < %s AND ip = %s', (attempt_data[1], attempt_data[0]))
    return True
